...
class Strategy(metaclass=ABCMeta):
    ...
    # ====== I() -- 指标注册 ======
    # 回测流程第1步: init() 里调用, 声明指标.框架会逐步揭示值(避免未来信息)
    # 并在图表上绘制.返回值是 _Indicator(_Array 子类).
    ...
    def buy(self, *, size: float = _FULL_EQUITY,
        ...
        """回测流程第2步(在 next() 中调用): 创建多头订单, 加入 Broker 队列等待撮合."""
...
class _Broker:
    ...
    def next(self):
        """回测流程第3步(每根 K 线): 撮合订单 → 记录权益 → 检测爆仓."""
        i = self._i = len(self._data) - 1
        self._process_orders()  # 核心撮合逻辑
    ...
    def _process_orders(self):
        """回测流程第3步核心----遍历订单队列按 stop→limit→market 优先级撮合: 
        1. 检查止损是否触发 2. 检查限价是否可达 3. 确定成交价 4. 开/平仓."""
        data = self._data
...
class Backtest:
    ...
    # ====== run() -- 执行回测(完整流程) ======
    # 流程: 创建数据/策略 → strategy.init() 预计算指标 →
    #       主循环逐 K 线: 揭示数据 → broker.next() 撮合 → strategy.next() 决策 →
    #       compute_stats() 输出统计
    ...
    # ====== optimize() -- 参数优化 ======
    # 对策略参数搜索最优组合.method='grid' 穷举笛卡尔积(支持多进程), 
    # method='sambo' 用贝叶斯优化(需 pip install sambo).
    def optimize(self, *,
